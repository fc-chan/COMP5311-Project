...
When a local video is uploaded, the live session metrics switch to a
content-aware utility model derived from that video's resolution and lightweight
frame analysis, so `Mean utility` and `QoE` can change across uploads.
...
The offline comparison table generated by `npm run eval` remains bitrate-only.
Uploaded-video effects appear in the live summary cards and showdown metrics,
not in the precomputed batch table.
